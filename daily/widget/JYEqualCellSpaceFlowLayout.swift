...
class JYEqualCellSpaceFlowLayout: UICollectionViewFlowLayout {
    ...
    func setCellFrame(with layoutAttributes : [UICollectionViewLayoutAttributes]){
        ...
        switch cellType {
        case AlignType.left:
            nowWidth = self.minimumInteritemSpacing
        ...
        case AlignType.center:
            nowWidth = (self.collectionView!.frame.size.width - sumCellWidth - (CGFloat(layoutAttributes.count - 1) * self.minimumInteritemSpacing)) / 2
            for attributes in layoutAttributes{
                var nowFrame = attributes.frame
                nowFrame.origin.x = nowWidth
                attributes.frame = nowFrame
                nowWidth += nowFrame.size.width + self.self.minimumInteritemSpacing
            }
            break;
        case AlignType.right:
            nowWidth = self.collectionView!.frame.size.width - self.minimumInteritemSpacing
